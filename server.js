...
try {
  ...
  GlobalFonts.registerFromPath(path.join(__dirname, 'fonts', 'BodoniModa-Italic.ttf'), 'Bodoni Moda');
...
- Output: square 1:1 format, photorealistic, no text, no watermarks`;
  ...
  const overlayPng = await createCanvasOverlay(SIZE, (ctx) => {
    ...
    // Caption text
    ctx.font = 'italic 84px "Bodoni Moda"';
    ctx.fillStyle = GOLD;
...
async function createBrandedImage(imageBuffer, captionText) {
  ...
  const overlayPng = await createCanvasOverlay(SIZE, (ctx) => {
    if (style === 0) {
      ...
      ctx.font = 'italic 58px "Bodoni Moda"';
      ctx.fillStyle = WHITE;
      ctx.shadowColor = 'rgba(0,0,0,0.8)';
      ctx.shadowBlur = 16;
      ctx.shadowOffsetX = 1;
